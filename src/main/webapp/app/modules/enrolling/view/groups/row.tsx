...
export const renderSchedule = (row: GroupsData) => {
...
export const renderLecturer = (row: GroupsData) => {
  return `${(row.lecturerTitle && `${row.lecturerTitle} `) || ' '}${(row.lecturerFirstName && `${row.lecturerFirstName} `) || ' '}${
    (row.lecturerSecondName && `${row.lecturerSecondName} `) || ''
  }${row.lecturerLastName || ' '}`;
};
  ...
  return (
    <React.Fragment>
      <TableRow className={classes.root}>
        ...
        <TableCell align="right">
          <span style={row.isLimitReached ? { color: red[500] } : { color: green[800] }}>{`${row.enrolledCount}/${row.limit}`}</span>
        </TableCell>
