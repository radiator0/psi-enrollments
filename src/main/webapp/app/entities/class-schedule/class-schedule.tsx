...
export class ClassSchedule extends React.Component<IClassScheduleProps> {
  componentDidMount() {
    this.props.getEntities();
  }

  render() {
    const { classScheduleList, match } = this.props;
    return (
      <div>
        <h2 id="class-schedule-heading">
          ...
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            ...
            &nbsp;
            <Translate contentKey="enrollmentsApp.classSchedule.home.createLabel">Create a new Class Schedule</Translate>
          </Link>
        </h2>
        <div className="table-responsive">
          {classScheduleList && classScheduleList.length > 0 ? (
            <Table responsive aria-describedby="class-schedule-heading">
              <thead>
                <tr>
                  <th>
              ...
              <tbody>
                {classScheduleList.map((classSchedule, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>
                      <TextFormat type="date" value={classSchedule.startTime} format={APP_DATE_FORMAT} />
                    </td>
                    <td>
                      <TextFormat type="date" value={classSchedule.endTime} format={APP_DATE_FORMAT} />
                    </td>
                    <td>
