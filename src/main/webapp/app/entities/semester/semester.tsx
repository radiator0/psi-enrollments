...
export class Semester extends React.Component<ISemesterProps> {
  componentDidMount() {
      ...
      <div>
        <h2 id="semester-heading">
          ...
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            ...
            &nbsp;
            <Translate contentKey="enrollmentsApp.semester.home.createLabel">Create a new Semester</Translate>
          </Link>
        </h2>
        <div className="table-responsive">
          {semesterList && semesterList.length > 0 ? (
            <Table responsive aria-describedby="semester-heading">
              <thead>
                <tr>
                  <th>
              ...
              <tbody>
                {semesterList.map((semester, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>
                      <TextFormat type="date" value={semester.startDate} format={APP_LOCAL_DATE_FORMAT} />
                    </td>
                    <td>
                      {semester.fieldOfStudyId ? (
                        <Link to={`field-of-study/${semester.fieldOfStudyId}`}>{semester.fieldOfStudyId}</Link>
                      ) : (
                        ''
                      )}
                    </td>
