...
export const EnrollmentRight = (props: IEnrollmentRightProps) => {
  useEffect(() => {
    props.getEntities();
  }, []);

  const { enrollmentRightList, match, loading } = props;
  return (
    <div>
      <h2 id="enrollment-right-heading">
        ...
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          ...
          &nbsp;
          <Translate contentKey="enrollmentsApp.enrollmentRight.home.createLabel">Create new Enrollment Right</Translate>
        </Link>
      </h2>
            ...
            <tbody>
              {enrollmentRightList.map((enrollmentRight, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {enrollmentRight.studentId ? <Link to={`student/${enrollmentRight.studentId}`}>{enrollmentRight.studentId}</Link> : ''}
                  </td>
...
const mapStateToProps = ({ enrollmentRight }: IRootState) => ({
  ...
  loading: enrollmentRight.loading,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(EnrollmentRight);
