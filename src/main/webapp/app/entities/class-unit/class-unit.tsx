...
export const ClassUnit = (props: IClassUnitProps) => {
  useEffect(() => {
    props.getEntities();
  }, []);

  const { classUnitList, match, loading } = props;
  return (
    <div>
      <h2 id="class-unit-heading">
        ...
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          ...
          &nbsp;
          <Translate contentKey="enrollmentsApp.classUnit.home.createLabel">Create new Class Unit</Translate>
        </Link>
      </h2>
            ...
            <tbody>
              {classUnitList.map((classUnit, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{classUnit.day ? <TextFormat type="date" value={classUnit.day} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{classUnit.startTime ? <TextFormat type="date" value={classUnit.startTime} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{classUnit.endTime ? <TextFormat type="date" value={classUnit.endTime} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>
...
export default connect(mapStateToProps, mapDispatchToProps)(ClassUnit);
